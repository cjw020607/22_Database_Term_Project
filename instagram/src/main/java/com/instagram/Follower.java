...
public class Follower {
    ...
    public Follower(Statement stmt, String m) {
        state = stmt;
        mode = m;
    }
    ...
    public void main() throws SQLException {
        ...
        // 다른 user의 follower를 볼 때
        if (mode == "other") {
            rs = state.executeQuery(
                    String.format("select* from follow where following_name = \'%s\'", browsed));
        }
        // 나의 follower를 볼 때
        else if (mode == "me") {
            rs = state.executeQuery(
                    String.format("select* from follow where following_name = \'%s\'", nickname));
        }
        String var;
        // 정보가 있으면 nickname만 가져와서 array에 넣음
        while (rs.next()) {
            var = rs.getString("nickname");
            // 값을 배열에 넣는 함수 호출
            array = Add(array, var);
        }

        ListTest listTest;
        String current = "Follower List";
        // follower list 가져오기
        if (array != null)
